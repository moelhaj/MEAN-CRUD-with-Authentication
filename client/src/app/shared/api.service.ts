...
export class ApiService {
  ...
  pushTicket(projectId, ticketId) {
    ...
    return this.http.put(`${config.pushTicketURI}/${projectId}`, ticket, httpOptions);
  }

  pullTicket(projectId, ticketId) {
    const ticket = {ticket: ticketId}
    const httpOptions = {
      headers: new HttpHeaders({
        'Content-Type': 'application/json',
    ...
    return this.http.put(`${config.pullTicketURI}/${projectId}`, ticket, httpOptions);
  }
  ...
  updateTicket(id, title, severity, details, user) {
    const ticket = {
      title,
      severity,
      details,
